...
void drawHistogram(const char* filename) {
    ...
    // Create a histogram
    TH2F *COLZ = new TH2F("COLZ", "Calculated kinetic energy vs simulated energy", 200, 0.2, 20.01, 200, 0.2, 30.01);


    // Fill the histogram from the branch
    tree->Draw(Form("(0.5*939*(100/8.988)*(1/(%s*%s))):%s>>COLZ", barTOFcorr, barTOFcorr, nInitEnergy), "barTOFcorr > 0 && goodEvent", "COLZ");
